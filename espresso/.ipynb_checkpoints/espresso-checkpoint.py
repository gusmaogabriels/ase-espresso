...
class Espresso(FileIOCalculator, object):
    ...
    def extract_noncollinear_magnetization(self, spin='all'):
        ...
        p = self.run_ppx('noncollmag.inp',
            inputpp=[['plot_num',13],['spidef calcun_component',s]],
            piperead=True, parallel=False)
...
class Espresso2(FileIOCalculator, object):
    """
    ASE interface for Quantum Espresso
    ...
    def __init__(self,
        ...
        self.prefix = 'qe_'+self.site.jobid
    ...
    def set_results(self, atoms):
        ...
        self._results = {'energy': self.read_energies()[0],
                        'forces': self.read_forces(),
                        'stress': self.read_stress(),
                        'absolute magnetization': self.read_magnetizations()[0],
                        'total magnetization': self.read_magnetizations()[1],
    ...
    def create_outdir(self):
        ...
        self.localtmp = self.site.make_localtmp(self.outdir)
        self.directory = self.localtmp
        self.scratch = self.site.make_scratch()

        if self.txt is None:
            self.log = self.localtmp.joinpath('pw.out')
        else:
            self.log = self.localtmp.joinpath(self.txt)
    ...
    def get_absolute_magnetization(self, atoms=None):
        ...
        self.update(atoms)
        return self._results.get('absolute magnetization', None)

    def get_total_magnetization(self, atoms=None):

        self.update(atoms)
        return self._results.get('total magnetization', None)
    ...
    def todb(self,only_nondefaults=True,identifier=None):
        """
        Coverts calculator object into a dictionary representation appropriate to be placed
        in a MongoDB. By default this returns only the settings that are not the default values.
        This function is based loosely off the todict function from the Kitchen Group's VASP
        ...
        only_nondefaults: bool
            If set to True, only the non-default keyword arguments are returned. If False all
            key word arguments are returned
        """
        ...
        #    dict_version['beefensemble'] = self.get_beef_ensemble()
        for prop in self.implemented_properties:
            val = self._results.get(prop, None)
            dict_version[prop] = val
        f = self._results.get('forces', None)
        if f is not None:
            dict_version['fmax'] = max(np.abs(f.flatten()))

        s = self._results.get('stress', None)
        if s is not None:
            dict_version['smax'] = max(np.abs(s.flatten()))
...
class iEspresso(Espresso):
        ...
        if self.site.batchmode:
            ...
            if self.calculation != 'hund':
                ...
                if not self._spawned:
                    self.child = pexpect.spawn(command,maxread=len(self.atoms)*((14+21)*3+4)*2,searchwindowsize=len(self.atoms)*((14+21)*3+4)*2)
                    self._spawned = True
                    self.child.logfile = self.logfile
                    self.child.logfile.write(self.get_output_header().encode('utf-8'))
                    self.child.logfile.write('command: {}'.format(command).encode('utf-8'))
                    print('__ spawning')
                    self.child.logfile.write('__ beef_ensemble = {}'.format(self.beefensemble).encode('utf-8'))
                    try:
                        print('READING IN, {}'.format(len(self.atoms))) 
                        i = self.child.expect(['\!ASE\r\n',
                                               'convergence NOT', 
                                               'stopping'],
                                              timeout=self.timeout)
                        if i == 0:
                            self.child.expect(['(([\s\+\-]{{14}}[\d\.eE\+\-]{{21}}){{3}}\r\n){{{}}}'.format(len(self.atoms)*2),pexpect.TIMEOUT],\
                                             timeout=self.timeout)
                        elif i == 1:
    ...
    def stop(self):
        ...
        if self._spawned:
            print('__ is spawned')
            self.child.send('Q\n')
            print('__ sent Q, {}'.format(self.beefensemble))
            if self.beefensemble:
                print('__ got in')
                i = self.child.expect(['\!BEEF\r\n',pexpect.TIMEOUT],timeout=self.timeout)
                i = self.child.expect(['JOB DONE\.\r\n',pexpect.TIMEOUT],timeout=self.timeout)
                print('__ got out')
            self.child.logfile.flush()
        else:
            print('__ is not spawned')
        #try
            #print(self.child.before)
            #print(self.child.after)
            #i = self.child.expect(['\!BEEF\_OUT\r\n',pexpect.TIMEOUT],timeout=self.timeout)
            #print('GOT OUT (should I?): {}, {}'.format(i, pexpect.TIMEOUT))
            #print(self.child.before)
            #print(self.child.after)
            #self.child.wait()
        #except:
        #     raise SCFTimeoutError() 
